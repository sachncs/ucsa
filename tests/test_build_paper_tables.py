...
def test_build_main_table_with_data():
    ...
    out = build_main_table(
        [{"data": {"best_val_ppl": 24.3}}, {"data": {"best_val_ppl": 25.7}}],
        rows,
    )
...
def test_build_ablation_table_groups_by_tag():
    out = build_ablation_table(
        [
            {"name": "ucsa-full-seed42.json", "data": {"best_val_ppl": 24.0}},
            {"name": "ucsa-full-seed43.json", "data": {"best_val_ppl": 26.0}},
            {"name": "ucsa-no-ema-seed42.json", "data": {"best_val_ppl": 28.0}},
        ]
    )
    assert "full" in out
